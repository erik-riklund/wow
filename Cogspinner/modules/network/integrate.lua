--    ____                      _                       
--   / ___|___   __ _ ___ _ __ (_)_ __  _ __   ___ _ __ 
--  | |   / _ \ / _` / __| '_ \| | '_ \| '_ \ / _ \ '__|
--  | |__| (_) | (_| \__ \ |_) | | | | | | | |  __/ |   
--   \____\___/ \__, |___/ .__/|_|_| |_|_| |_|\___|_|   
--              |___/    |_|                            
--
--- @type string, core.context
local addon, framework = ...

-- #region: framework context imports

--- @type network.invokeChannel
local invokeChannel = framework.import('channel/invoke')

--- @type network.reserveChannel
local reserveChannel = framework.import('channel/reserve')

--- @type network.registerListener
local registerListener = framework.import('channel/register-listener')

--- @type network.removeListener
local removeListener = framework.import('channel/remove-listener')

--- @type plugins.retrievePluginContext
local retrievePluginContext = framework.import('plugin/retrieve')

-- #endregion

---
--- The API object providing methods for plugins to interact
--- with communication channels within the framework.
---
--- @type network.pluginApi
---
local api = {
  invokeChannel = function(self, name, payload)
    invokeChannel(name, payload, retrievePluginContext(self.identifier))
  end,

  registerChannelListener = function(self, channel, listener)
    registerListener(channel, listener, retrievePluginContext(self.identifier))
  end,

  removeChannelListener = function(self, channel, identifier)
    removeListener(channel, identifier, retrievePluginContext(self.identifier))
  end
}

--
-- A listener triggered when a new plugin is added to the system. It extends the
-- plugin's capabilities by adding methods for interacting with channels.
--
-- Additionally, it reserves channels specified in the plugin's options.
--
registerListener('PLUGIN_ADDED', {
  --- @param plugin plugin
  --- @param options plugins.createPlugin.options
  callback = function(plugin, options)
    -- ~ Integrate the API methods into the plugin.

    plugin.invokeChannel = api.invokeChannel
    plugin.registerChannelListener = api.registerChannelListener
    plugin.removeChannelListener = api.removeChannelListener

    -- ~ Reserves channels specified in the plugin's options,
    -- ~ ensuring they are available for the plugin to use.

    if type(options.channels) == 'table' then
      for index, channel in ipairs(options.channels) do
        reserveChannel(channel.name, {
          async = channel.async,
          internal = channel.internal,
          owner = plugin
        })
      end
    end
  end
})
