---------------------------------------------
-- Ghuul Addons: Selective Autoloot v1.7.1 --
---------------------------------------------
...
SA:SetScript("OnEvent", function(_,EVENT)
    ...
    if EVENT == "PLAYER_LOGIN" then
        ...
        if SAConfig["JUNK_SKINNING_OVERRIDE"] == 1 and SAConfig["JUNK_MINPRICE"] > 1 then
            ...
            local Profession1 = "" local Profession2 = ""
...
function SA_ProcessLoot()
    for i = ItemCount,1,-1 do
        ...
        if not Locked and not tContains(SAItemList["Ignore"][Item["Expansion"]],Item["Name"]) then
            -- Currency:
            if SlotType == 3 and not tContains(SAConfig["CURRENCY_IGNORE"],Item["Name"]) then SA_LootSlot(i)
            
            -- Quest-related items:
            elseif QuestItem and ((ItemCount == 1 or SAConfig["QUEST_LOOT_ALL"] == 1) or
                tContains(SAItemList["Quests"],Item["Name"])) then SA_LootSlot(i)
            
            -- Coins:
            elseif SlotType == 2 then
                local Cash = strsplit("\n",Item["Name"])
                local CashAmount,CashValue = strsplit(" ",Cash)
                if CashValue ~= "Gold" or (CashValue == "Gold" and tonumber(CashAmount) < SAConfig["GOLD_LOOT_MAX"]) then SA_LootSlot(i) end
            ...
            elseif SA_CurrentItemShouldBeLooted() then SA_LootSlot(i) end
        end
...
function SA_CurrentItemShouldBeLooted()
    ...
    if tContains(SAItemList["Custom"][Item["Expansion"]],Item["Name"]) then return true
    
    -- Is the item used for Enchanting?
    elseif Item["Type"] == "Tradeskill" and Item["Subtype"] == "Enchanting" then return true
